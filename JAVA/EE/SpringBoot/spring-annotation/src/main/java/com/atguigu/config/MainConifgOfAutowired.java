 ...
 * 		2）、如果找到多个相同类型的组件，再将@Autowired修饰的属性的名称作为bean的name去容器中查找
 * 		3）、自动装配默认一定要将属性赋值好，没有就会报错；可以使用@Autowired(required=false);防止报错
 * 		BookService{
 * 			@Autowired
 * 			BookDao  bookDao2;
 * 		}
...
public class MainConifgOfAutowired {
	...
	/**
	 * @Bean标注的方法创建对象的时候，方法参数中的bean的值 也是可以自动 从ioc容器中获取
	 * @param car
	 * @return
	 */
	@Bean
