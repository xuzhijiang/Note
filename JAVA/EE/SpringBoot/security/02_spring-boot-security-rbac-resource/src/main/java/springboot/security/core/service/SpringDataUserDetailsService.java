...
public class SpringDataUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(final String username) throws UsernameNotFoundException {
		...
		//根据用户的id查询用户的权限  (t_permission表)
		List<String> permissions = userDao.findPermissionsByUserId(userDto.getId());
		String[] permissionArray = new String[permissions.size()]; //将permissions转成数组
		permissions.toArray(permissionArray);

		// 如果需要用户的角色,也可以查询出这个用户的角色  (t_role表)
		...
		UserDetails userDetails = User.withUsername(userDto.getUsername())
				...
				.authorities(permissionArray) // 基于资源的访问控制
