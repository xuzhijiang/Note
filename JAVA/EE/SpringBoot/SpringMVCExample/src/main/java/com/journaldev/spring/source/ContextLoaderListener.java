    ...
    public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
        ...
        try {
            if (this.context == null) {
                // 3 创建WebApplicationContext实现类的实例。其内部首先会确定WebApplicationContext实例类型。
                // 首先判断有没有<context-param>元素的<param-name>值为contextClass，
                // 如果有，则对应的<param-value>值，就是要创建的WebApplicationContext实例类型
            ...
            if (this.context instanceof ConfigurableWebApplicationContext) {
                ...
                // 4.1 由于WebApplicationContext对象实例还没有加载对应配置文件，
                // spring上下文还没有被刷新，因此isActive返回false，进入if代码块
                if (!cwac.isActive()) {
                    //4.2 当前ROOT WebApplicationContext的父context为null，
            ...
            // 以”org.springframework.web.context.WebApplicationContext.ROOT”为key设置到ServletContext中
