 ...
 * 使用非阻塞式实现(重点)(非阻塞网络通信)
 *
 * 选择器（Selector）：将每一个通道注册到选择器上，选择器监控每一个通道的IO状况（读，写，连接等情况）。
...
public class TestBlockingNIOServer4 {
    ...
    public static void server() throws IOException{
        ...
        serverSocketChannel.register(selector, SelectionKey.OP_ACCEPT);// 这里是注册这个通道，并且让selector监听这个通道的接受事件.
        System.out.println("after register----");
        // 如果注册时不止监听一个事件，则可以使用“位或”操作符连接。如下
        // int interestSet = SelectionKey.OP_RED|SelectionKey.OP_WRITE;

        // 通过轮询式的方式获取选择器中准备就绪的事件
        System.out.println("00000 selector.select(): ");
        // 注意这个select()方法会阻塞.
        while(selector.select() > 0){
            System.out.println("11111 selector.select(): ");
            // 获取当前选择器中所有注册的选择键（已就绪的监听事件）
            Iterator<SelectionKey> it = selector.selectedKeys().iterator();
            System.out.println("2222 it.hasNext(): " + it.hasNext());
            while (it.hasNext()) {
                //获取准备就绪的事件
