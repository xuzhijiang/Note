...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if(userID.equals(user) && password.equals(pwd)){
			...
			// 我们为响应设置的cookie, 然后将其转发(forwarding it)到LoginSuccess.jsp
			// 此cookie将用于跟踪会话， 另请注意，cookie超时设置为30分钟。
			
			// 另外： 理想情况下，应该有一个复杂的逻辑来设置cookie值,以便session跟踪，以便它不会与任何其他请求冲突。
			// 也就是cookie的生成算法，避免不同请求的cookie相同
			
			response.addCookie(loginCookie);// 将指定的cookie添加到响应中。 可以多次调用此方法来设置多个cookie。
