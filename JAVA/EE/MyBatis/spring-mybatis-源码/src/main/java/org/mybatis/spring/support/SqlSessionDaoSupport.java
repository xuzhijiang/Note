...
public abstract class SqlSessionDaoSupport extends DaoSupport {
  ...
  /**
   * 在ClassPathMapperScanner中设置了注入模式为AUTOWIRE_BY_TYPE,所以spring ioc容器会调用这个set方法.为sqlSessionTemplate赋值
   *
   * Set MyBatis SqlSessionFactory to be used by this DAO. Will automatically create SqlSessionTemplate for the given
   * SqlSessionFactory.
   *
   * @param sqlSessionFactory 调用这个set方法的参数的SqlSessionFactory的bean是哪里来的? 怎么找到的?
   *                          从容器中获取的我们自己配置的SqlSessionFactory的bean,因为在ClassPathMapperScanner中设置了注入模式为AUTOWIRE_BY_TYPE,
   *                          所以会根据SqlSessionFactory这个TYPE在ioc容器中寻找SqlSessionFactory类型的bean,
   *                          注意如果在ClassPathMapperScanner中设置了注入模式为AUTOWIRE_BY_NAME,就要根据name为sqlSessionFactory的bean,去容器中寻找了.
   *          a factory of SqlSession
   */
  public void setSqlSessionFactory(SqlSessionFactory sqlSessionFactory) {
    /**
     * 为sqlSessionTemplate赋值
     */
