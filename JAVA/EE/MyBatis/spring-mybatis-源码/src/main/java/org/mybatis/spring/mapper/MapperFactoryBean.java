...
public class MapperFactoryBean<T> extends SqlSessionDaoSupport implements FactoryBean<T> {
  ...
  /**
   * 在这里又是一个牛逼的闪光点
   * mapper接口是不可以实例化的,所以要用jdk的动态代理生成一个mapper接口的代理对象,所以这个构造器接受的参数
   * mapperInterface就是mapper接口类,在下面getObject()方法返回动态代理对象的时候会用到
  ...
  /**
   * 实现FactoryBean接口,就会调用这个方法,把这个方法的返回值返回.
   * 这里就是通过这个方法,返回mapper接口的代理对象
   */
  @Override
  public T getObject() throws Exception {
    /**
     * 第一步: 获取我们的SqlSessionTemplate
     * 第二步: 获取SqlSessionTemplate.getMapper(mapperInterface)
     */
