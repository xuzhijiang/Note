...
public class MapperScannerConfigurer
   ...
   * {@inheritDoc}
   *
   * 方法说明: 注册@MapperScan注解所扫描的mapper包下的bean定义到容器中
   *
  ...
  public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) {
    /**
     * 当processPropertyPlaceHolders为true时,就进行processPropertyPlaceHolders();
     */
    if (this.processPropertyPlaceHolders) {
      processPropertyPlaceHolders();
    }
    /**
     * 显示的new一个ClassPathMapperScanner 包扫描器对象,这个对象是mybatis继承了spring的
     * ClassPathBeanDefinitionScanner, 为我们扫描器指定@MapperScan属性
     */
