...
// 用 ZooKeeper 充当一个 服务注册表（Service Registry），让多个 服务提供者（Service Provider）形成一个集群，
// 将Service Provider中的服务注册到ZooKeeper这个服务注册表中,(实际注册到ZNode中).
public class ServiceProvider {

    private static final Logger LOGGER = LoggerFactory.getLogger(ServiceProvider. class);

    // 用于等待 SyncConnected 事件触发后继续执行当前线程
    // CountDown: 倒计时  Latch: 闩,锁，门闩
    private CountDownLatch latch = new CountDownLatch(1);

    // 发布 RMI 服务,并注册 RMI 地址到 ZooKeeper 中
    public void publish(Remote remote , String host, int port) {
        // 发布 RMI 服务并返回 RMI 地址
        String url = publishService(remote , host , port );

        // 注册 RMI 地址到 ZooKeeper 中
        if (url != null) {
