Java Queue是java.util包中可用的接口，并扩展了java.util.Collection接口。
就像Java List一样，Java Queue是有序元素（or object）的集合，但它以不同方式执行插入和删除操作。 
(it performs insert and remove operations differently.)在处理这些元素之前，
我们可以使用Queue存储元素。
...
1. java.util.Queue接口是java.util.Collection接口的子类型。
2. 就像真实世界(real-world)的队列一样，Queue在队列末尾(rear)插入(insert)
元素并从队列的开头(front)删除(delete)。   (insert)--rear-----------front--->(remove)
3. Java Queue表示元素的有序列表。(represents an ordered list of elements.)
4. Java Queue遵循FIFO顺序来插入和删除它的元素。 FIFO代表先入先出。(FIFO stands for First in First Out)
5. Java Queue支持Collection接口的所有方法。
