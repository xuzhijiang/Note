...
public class MutexImpl implements Mutex{
    ...
    // 自定义同步组件将使用同步器AbstractQueuedSynchronizer
    // 提供的模板方法来实现自己的同步语义。
...
//    }
    ...
    public void lock() {
        ...
        // 则由该方法返回，否则，将会进入同步队列等待，该方法将会调用重写的tryAcquire(int arg)方法
        // 这是一个获取执行许可的方法，arg表示获取许可的数量

        // 可用许可的数量的支持：它使用了1个一个int类型state变量，表示同步状态，也就是说笔者所说"执行许可"。
    ...
    // 在我们编写一个同步组件的时候，例如我们想实现一个独占锁，假设为Sync，其继承了AQS。
    ...
    // 同步组件推荐定义为静态内部类：因为某个同步组件通常是为实现特定的目的而实现，
