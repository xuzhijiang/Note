...
public class ArrayListSource<E> {
    ...
    private static final Object[] EMPTY_ELEMENTDATA = {}; // 一个空数组，容量为0

    private transient Object[] elementData; // 存储集合数据的内部数组，默认值为null

    // ArrayList集合中数组的当前有效长度，比如数组的容量是5,
    // size是1 表示容量为5的数组目前已经有1条记录了，其余4条记录还是为空
    private int size;
    ...
    public ArrayListSource() { // 没有参数的构造函数
        super(); // 调用父类AbstractList的方法构造函数
        // 让elementData和ArrayList的EMPTY_ELEMENTDATA这个空数组使用同一个引用
