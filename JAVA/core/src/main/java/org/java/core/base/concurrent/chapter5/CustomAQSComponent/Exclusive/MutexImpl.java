 ...
 * 独占式组件:
 *
 * 第一步: 如果你打算在自己的独占式组件MutexImpl中调用
 * 内部类Sync的 acquire(int arg) 或者 acquireInterruptibly(int arg) 或者 tryAcquireNanos(int arg, long nanosTimeout)这三个AQS的模板方法,
 * 那么你就需要在Sync中重写AbstractQueuedSynchronizer的tryAcquire(int arg),在
 * tryAcquire(int arg)中利用CAS改状态,如果Sync不实现tryAcquire这个方法,则会抛出UnsupportedOperationException
 *
 * 第二步: 如果你打算在自己的独占式组件MutexImpl中调用内部类Sync的release,
 * 就需要Sync内部类重写AQS的tryRelease方法,来释放锁,如果不实现，则抛出UnsupportedOperationException
 */
