...
1. AbstractQueuedSynchronizer是JDK 1.5引入的，用来构建锁或者其他同步组件的基础，通常简称为AQS。
...
4. Java中信号量（Semaphore）、读写锁（ReadWriteLock）、计数器（CountDownLatch）以及FutureTask等都是基于AQS实现的，可见这个抽象类的地位多么不一般
...
// 方法如果不实现，则抛出UnsupportedOperationException异常
...
// 需要覆写的方法:
// 独占式释放同步状态,如果不实现，则抛出UnsupportedOperationException异常
...
boolean tryAcquireSharedNanos(int arg, long nanosTimeout)
// 以上3个方法，需要覆写的方法:
// 共享式获取同步状态。如果不实现，则抛出UnsupportedOperationException异常
...
// 需要覆写的方法
// 共享式释放同步状态。如果不实现，则抛出UnsupportedOperationException异常
boolean tryReleaseShared(int arg)
```

>此外，还需要一个isHeldExclusively()方法需要覆写(查询当前同步器是否在独占模式下被占用)，不过其没有对应的模板方法。通常情况下，
...
>案例:org.java.core.base.concurrent.chapter5.CustomAQSComponent.MutexImpl.java