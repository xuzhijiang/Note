...
![](pics/java7-最重要参数.png)
    ...
    -XX:PermSize -XX:MaxPermSize是java7的,这辈子基本用不到了,了解下就行
    
![](pics/java8-重要参数.png)

![](pics/java8-元空间.png)

- `-Xms64m` 初始堆内存 `64m` (Xms: start)
- `-Xmx128m` 最大堆内存 `128m` (Xmx: max)
...
新生代和老年代的默认比例为 `1:2`,可以通过参数 `-XX:NewRatio=2` 来设置老年代/新生代的比例。
...
![](pics/OOMError分析.png)
    ...
    示例: org.java.core.advanced.jvm.ErrorAndException
