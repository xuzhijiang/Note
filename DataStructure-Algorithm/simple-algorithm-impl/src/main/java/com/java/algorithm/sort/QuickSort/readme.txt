com.java.algorithm.sort.QuickSort
...
最好: O(n), 当前划分均匀, 当分组过程始终将中间元素选为枢轴时，会出现最佳情况。

平均: O(nlogn)

最差: O(n^2) -> 最坏的情况发生在分组过程中总是选择最大或最小元素作为哨兵时.
 如果我们考虑上面的策略，我们选择最后一个元素做为哨兵，那么当数组已经按递增或递减顺序排序时，最坏的情况就会发生。 
...
默认实现不稳定。 但是，通过将索引视为比较参数，可以使任何排序算法稳定。