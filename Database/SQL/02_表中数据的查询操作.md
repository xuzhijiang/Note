...
5. 如果不加括号，条件运算按照NOT、AND、OR的优先级进行，即NOT优先级最高，其次是AND，最后是OR。可以使用 () 来决定优先级，使得优先级关系更清晰.
6. <>或者!=: 不等于
7. =,>,<,>=,<=: 等于，大于，小于，大于等于，小于等于
8. BETWEEN: 在2个值之间,例如: BETWEEN 1 AND 20;包括1和20.
9. IS NULL: 为 NULL 值(NULL 与 0、空字符串都不同)
